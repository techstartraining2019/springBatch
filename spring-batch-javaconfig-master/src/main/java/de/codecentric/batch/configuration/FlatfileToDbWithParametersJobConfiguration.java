...
public class FlatfileToDbWithParametersJobConfiguration {
  ...
  public Job flatfileToDbWithParametersJob() {
    return jobBuilders.get("flatfileToDbWithParametersJob").listener(protocolListener())
        .start(step()).build();
  }

  @Bean
  public Step step() {
    return stepBuilders.get("step").<Partner, Partner>chunk(1)
        .reader(reader(OVERRIDDEN_BY_EXPRESSION)).processor(processor()).writer(writer())
        .listener(logProcessListener()).stream(stream()).build();
  }
  ...
  public FlatFileItemReader<Partner> reader(
      @Value("#{jobExecutionContext[pathToFile]}") String pathToFile) {
