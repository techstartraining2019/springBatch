...
public class CustomStepBuilderFactory<I, O> {
  ...
  public CustomStepBuilderFactory(JobRepository jobRepository,
      PlatformTransactionManager transactionManager, CompletionPolicy completionPolicy,
      ItemReader<I> itemReader, ItemProcessor<I, O> itemProcessor, ItemWriter<O> itemWriter,
      StepListener... stepListeners) {
  ...
  public SimpleStepBuilder<I, O> get(String name) {
    SimpleStepBuilder<I, O> builder = new StepBuilder(name).repository(jobRepository)
        .transactionManager(transactionManager).<I, O>chunk(completionPolicy).reader(itemReader)
