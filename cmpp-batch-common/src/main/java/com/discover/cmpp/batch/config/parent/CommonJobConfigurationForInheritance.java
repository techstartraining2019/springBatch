...
import com.discover.cmpp.batch.config.BatchProperties;
import com.discover.cmpp.batch.config.InfrastructureConfiguration;
import com.discover.cmpp.batch.domain.Partner;
import com.discover.cmpp.batch.listener.LogProcessListener;
import com.discover.cmpp.batch.listener.ProtocolListener;
...
public abstract class CommonJobConfigurationForInheritance {
  ...
  /*
   * @Autowired private InfrastructureConfiguration infrastructureConfiguration;
   */
  ...
  protected CustomStepBuilderFactory<Partner, Partner> customStepBuilders() {
    ...
    return new CustomStepBuilderFactory<Partner, Partner>(jobRepository, transactionManager,
        completionPolicy(), reader(), processor(), writer(), logProcessListener());
  }
  ...
  public FlatFileItemReader<Partner> reader() {
    ...
    itemReader
        .setResource(new FileSystemResource(batchProperties.getInputFileProperties().get("FILEPATH")
            + batchProperties.getInputFileProperties().get("FILENAME")));
    return itemReader;
  }
